...
serve(async (req) => {
  ...
  try {
    ...
    if (!RAWG_API_KEY) {
      console.error(
        `[${requestId}] Configuration error: RAWG_API_KEY is not set`,
      );
    ...
    console.log(
      `[${requestId}] Proxying to RAWG: ${rawgUrl.pathname}${rawgUrl.search.replace(RAWG_API_KEY, 'REDACTED')}`,
    );

    const response = await fetch(rawgUrl.toString());
    console.log(
      `[${requestId}] RAWG response: ${response.status} ${response.statusText}`,
    );
    ...
    if (contentType && contentType.includes('application/json')) {
      ...
      console.warn(
        `[${requestId}] Non-JSON response (${response.status}):`,
        text.slice(0, 200),
      );
      data = {
        error: 'RAWG returned a non-JSON response',
